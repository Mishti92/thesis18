...
nbfilter = int(sys.argv[1]) 
print "no of filters ", nbfilter 
...
def run_network_new(nbfilter, name, fixed_weights, training, y, validation, val_y, batch_size=50, nb_epoch=30):
...
#     three filters corresponsing pwm length of 4, 7 and 11

    tower_1 = Conv1D(nbfilter, 4, padding='valid', activation='relu')(input_shape)    
    tower_1 = BatchNormalization()(tower_1)
    tower_1 = MaxPooling1D(pool_size=3, padding='valid')(tower_1)

    tower_2 = Conv1D(nbfilter, 7, padding='valid', activation='relu')(input_shape)
    tower_2 = BatchNormalization()(tower_2)
    tower_2 = MaxPooling1D(pool_size=3, padding='valid')(tower_2)
    
    tower_3 = Conv1D(nbfilter, 11, padding='valid', activation='relu')(input_shape)
    tower_3 = BatchNormalization()(tower_3)
    tower_3 = MaxPooling1D(pool_size=3, padding='valid')(tower_3)
    
    tower=[tower_1,tower_2, tower_3]
    merged = concatenate(tower, axis=1)
#     The model has a LSTM hidden layer to extract features from the sequence, followed by a fully connected layer to interpret the LSTM output, followed by an output layer for making binary predictions.

    lstm = Bidirectional(LSTM(64))(merged)
...
kernel_size=[]
for key in keyList:
    rbp_len= len(PWM[key]['a'])
    kernel_size.append(rbp_len)
    temp=[]
    for i in range(rbp_len):
        temp.append([[PWM[key]['a'][i]],[PWM[key]['c'][i]],[PWM[key]['g'][i]],[PWM[key]['u'][i]]])
    fixed_weights.append([array(temp),array([0])])
...
#for i in range(1):
    ...
    name = 'cnn1Lstm+3trainableFilters'
    batch_size= 50 
    nb_epoch = 50
    ...
    start = time.time()
    model, history = run_network_new(nbfilter, name, fixed_weights, seq_train, y, validation = seq_validation,val_y = val_y, batch_size=batch_size,
                                     nb_epoch = nb_epoch)
    end = time.time()
