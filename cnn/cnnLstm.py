...
for i in range(1):
    ...
    start = time.time()
    model, history = run_network_new(name, seq_train, y, validation = seq_validation,val_y = val_y, batch_size=batch_size,
                                     nb_epoch = nb_epoch)
    end = time.time()
    ...
    outfile="outfile/predictions%s_CLASS_1.txt"%(protein)
    fw = open(outfile, 'w')
    myprob = "\n".join(map(str, predictions[:, 1]))
    fw.write(myprob)
    fw.close()
    
    outfile="outfile/predictions%sCLASS_0.txt"%(protein)
    fw = open(outfile, 'w')
